...
export const createACLPlaywrightTest = ({ server = false, route = '/', moduleUrl = null } = {}) =>
    base.extend({
        aclServer: [
            async ({}, use) => {
                if (!server) {
                ...
                const { startACLTestServer } = await import('../server/testing-server.mjs'),
                    app = await startACLTestServer(typeof server === 'object' ? server : {});
                // Run this operation
                try {
                    await use(app);
                } finally {
        ...
        acl: async ({ page, aclServer }, use) => {
            ...
            if (!entry)
                throw new TypeError('[ACL Testing] createACLPlaywrightTest() requires server options or moduleUrl.');
