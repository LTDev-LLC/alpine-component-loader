...
const currentModuleUrl = new URL(import.meta.url),
    ...
    pendingDefinitionFailuresByTag = new Map(),
    pendingConfiguration = [],
    pendingBuiltIns = new Map(),
    facadeManifestDependencies = new Map(),
    facadeManifestGroups = new Map(),
    facadeDetachedDataEntries = new Set(),
    normalizeCacheNamespace = (value) =>
...
let implementationPromise = null,
    ...
    facadeReportOverride = null,
    facadeStarted = false,
    facadeDisposed = false;
...
const validateFacadeConfiguration = (settings) => {
    const validCacheStrategies = new Set(['cache-first', 'network-first', 'stale-while-revalidate', 'no-store']),
        validLoadingModes = new Set(['eager', 'lazy', 'idle']),
        validHydrationModes = new Set(['eager', 'visible', 'idle', 'interaction', 'media']),
        validResponseTypes = new Set(['json', 'text', 'blob', 'arrayBuffer', 'stream', 'auto']),
        data = settings.data && typeof settings.data === 'object' ? settings.data : {};
    if (settings.templateCacheStrategy != null && !validCacheStrategies.has(settings.templateCacheStrategy))
        throw new TypeError(`[ACL] Unsupported template cache strategy "${settings.templateCacheStrategy}".`);
    if (settings.loading != null && !validLoadingModes.has(settings.loading))
        throw new TypeError(`[ACL] Unsupported loading mode "${settings.loading}".`);
    if (settings.hydrate != null && !validHydrationModes.has(settings.hydrate))
        throw new TypeError(`[ACL] Unsupported hydration mode "${settings.hydrate}".`);
    if (settings.hydrate === 'media' && !(typeof settings.hydrateMedia === 'string' && settings.hydrateMedia.trim()))
    ...
    ['persistDebounce', 'runtimeCacheMax', 'transitionDuration', 'templateCacheTtl', 'templateCacheMax'].forEach(
        (key) => {
            ...
            if (settings[key] != null && (!Number.isFinite(Number(settings[key])) || Number(settings[key]) < 0))
                throw new TypeError(`[ACL] ${key} must be a non-negative finite number.`);
        },
    );
    if (
        settings.keepAliveMax !== Infinity &&
        settings.keepAliveMax != null &&
        (!Number.isFinite(Number(settings.keepAliveMax)) || Number(settings.keepAliveMax) < 0)
    )
        throw new TypeError('[ACL] keepAliveMax must be a non-negative number or Infinity.');
    ['poll', 'timeout', 'cacheTtl', 'cacheMax', 'retries', 'retryDelay', 'retryMaxDelay'].forEach((key) => {
        // Validate every non-negative grouped data setting
        if (data[key] != null && (!Number.isFinite(Number(data[key])) || Number(data[key]) < 0))
            throw new TypeError(`[ACL] data.${key} must be a non-negative finite number.`);
    });
    if (
        data.retryJitter != null &&
        (!Number.isFinite(Number(data.retryJitter)) || Number(data.retryJitter) < 0 || Number(data.retryJitter) > 1)
    )
        throw new TypeError('[ACL] data.retryJitter must be between 0 and 1.');
    if (data.responseType != null && !validResponseTypes.has(data.responseType))
        throw new TypeError(`[ACL] Unsupported data.responseType value "${data.responseType}".`);
    if (data.cacheStrategy != null && !validCacheStrategies.has(data.cacheStrategy))
        throw new TypeError(`[ACL] Unsupported data.cacheStrategy value "${data.cacheStrategy}".`);
    if (
        settings.persistVersion != null &&
        (!Number.isInteger(Number(settings.persistVersion)) || Number(settings.persistVersion) < 1)
    )
        throw new TypeError('[ACL] persistVersion must be a positive integer.');
};
...
const snapshotConfiguration = (options) => {
    ...
    ['data', 'hooks', 'attributes', 'errorCss', 'events', 'security', 'observability', 'adaptivePrefetch'].forEach(
        (key) => {
            ...
            if (options[key] && typeof options[key] === 'object') snapshot[key] = { ...options[key] };
        },
    );
    return snapshot;
};
...
const syncImplementationState = async (Implementation) => {
    const namespace = normalizeCacheNamespace(AlpineComponentLoader.globalConfig.cacheNamespace),
        prefix = `alpine-component-loader-${namespace}-`;
    ...
    if (AlpineComponentLoader.toggleDebug !== defaultToggleDebug)
        Implementation.toggleDebug = AlpineComponentLoader.toggleDebug;
...
const loadImplementation = () => {
    ...
    const loading = importDeferredLocalModule('./runtime/loader.js')
        .then(async ({ default: Implementation }) => {
...
const builtInSelector = (name) =>
    name === 'declarative'
        ? AlpineComponentLoader.globalConfig.defaultComponentName || 'acl-component'
        : name === 'dynamic'
          ? AlpineComponentLoader.globalConfig.defaultDynamicName || 'acl-dynamic'
          : AlpineComponentLoader.globalConfig.defaultBoundaryName || 'acl-boundary';
...
const registerBuiltIn = async (name, elementName = builtInSelector(name)) => {
    ...
    const registration = loadImplementation()
...
const scanBuiltIns = (root) => {
    ...
    const declarativeName = builtInSelector('declarative'),
        dynamicName = builtInSelector('dynamic'),
        boundaryName = builtInSelector('boundary'),
        registrations = [];
    if (root.matches?.(declarativeName) || root.querySelector(declarativeName))
        registrations.push(registerBuiltIn('declarative', declarativeName));
    if (root.matches?.(dynamicName) || root.querySelector(dynamicName))
        registrations.push(registerBuiltIn('dynamic', dynamicName));
    if (root.matches?.(boundaryName) || root.querySelector(boundaryName))
        registrations.push(registerBuiltIn('boundary', boundaryName));
...
const installBuiltInDiscovery = () => {
    ...
    const root = AlpineComponentLoader.root || document,
        initialRegistrations = scanBuiltIns(root);
    ...
    discoveryObserver = new MutationObserver((records) => {
        ...
        records.forEach((record) => {
            ...
            record.addedNodes.forEach((node) => {
                ...
                if (node.nodeType === 1)
                    scanBuiltIns(node).forEach(
                        // Report background discovery failures without creating unhandled rejections
                        (registration) =>
                            void registration.catch((error) =>
                                AlpineComponentLoader._report(
                                    'warn',
                                    '[ACL] Failed to register a discovered built-in element.',
                                    error,
                                    { phase: 'runtime-import' },
                                ),
                            ),
...
const drainPendingDefinitions = async () => {
    ...
    while (pendingDefinitions.size) await Promise.allSettled([...pendingDefinitions]);
...
export default class AlpineComponentLoader {
    ...
    static config(options) {
        ...
        if (implementationClass) {
            ...
            const staged = {
                ...
                data: { ...(AlpineComponentLoader.globalConfig.data || {}) },
            };
    ...
    static async start() {
        ...
        facadeStartingPromise = (async () => {
            ...
            if (implementationClass || hasTemplates) {
                ...
                await drainPendingDefinitions();
                Implementation._registerDefinitions();
                if (Implementation.globalConfig.adaptivePrefetch)
                    await Implementation.observePrefetch(
                        Implementation.globalConfig.adaptivePrefetch === true
                            ? {}
                            : Implementation.globalConfig.adaptivePrefetch,
                    );
            }
            facadeStarted = true;
        })();
    ...
    static define(tagName, source, config) {
        ...
        void operation.then(
            // Remove a successfully registered pending definition
            () => {
                pendingDefinitions.delete(operation);
                if (pendingDefinitionsByTag.get(definitionKey) === operation)
                    pendingDefinitionsByTag.delete(definitionKey);
            },
    ...
    static async registerTemplates(root = document) {
        AlpineComponentLoader._assertActive();
        const Implementation = await loadImplementation(),
            constructors = await Implementation.registerTemplates(root);
    ...
    static observeTemplates(options = {}) {
        ...
        AlpineComponentLoader._assertActive();
        const root = options.root ?? AlpineComponentLoader.root ?? document.documentElement,
    ...
    static getMetrics() {
        return (
            implementationClass?.getMetrics() || {
                startedAt: Date.now(),
                totals: {},
                durations: {},
                recent: [],
    ...
    static getDataCacheInfo(finalUrlOrKey = null) {
        return (
            implementationClass?.getDataCacheInfo(finalUrlOrKey) ||
            (finalUrlOrKey
                ? null
                : {
                      size: 0,
    ...
    static async dispose({ clearPersistentCaches = true } = {}) {
        ...
        if (implementationClass)
            await implementationClass.dispose({
...
const isolatedAsyncMethodDescriptors = [
    ...
    disposeStagedSubscriptions = (subscriptions) => {
        ...
        subscriptions.forEach(
            // Release one staged subscription
    ...
    installAsyncForwarders = (Facade, ready) => {
        ...
        isolatedAsyncMethodDescriptors.forEach(
            // Install one asynchronous method descriptor
            (name) =>
                Object.defineProperty(Facade, name, {
                    ...
                    value: (...args) => {
                        ...
                        return ready.then(
                            // Forward arguments after isolated activation
...
export const createLoader = ({ root = null, config = {}, cacheNamespace = null } = {}) => {
    const instanceId = `instance-${Date.now().toString(36)}-${++isolatedLoaderSequence}`,
    ...
    class IsolatedLoader {
        ...
        static observeTemplates(options = {}) {
            ...
            void ready.then((Loader) => {
                if (!disposed)
                    release = Loader.observeTemplates({
        ...
        static getDataCacheInfo(key = null) {
            return (
                activeLoader?.getDataCacheInfo(key) ||
                (key
                    ? null
                    : {
...
export const createIndexedDBPersistenceAdapter = (...args) => {
    ...
    const getAdapter = () => {
        if (!adapterPromise) {
            const loading = importDeferredLocalModule('./runtime/persistence.js')
                .then(
                    // Construct the concrete adapter after its module arrives
                    ({ createIndexedDBPersistenceAdapter: createAdapter }) => createAdapter(...args),
                )
                .catch((error) => {
