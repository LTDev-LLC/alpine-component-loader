...
const moduleSuffix = new URL(import.meta.url).pathname.endsWith('.min.js') ? '.min.js' : '.js',
    ...
    { hasOwn } = await importLocalModule('./config.js');

const DEFAULT_INDEXED_DB_NAME = 'alpine-component-loader',
    DEFAULT_INDEXED_DB_STORE = 'persistence';

// Open an IndexedDB database and create the persistence store during upgrades
const openIndexedDB = (indexedDBImpl, databaseName, storeName, version = null) =>
...
export const createIndexedDBPersistenceAdapter = ({
    databaseName = DEFAULT_INDEXED_DB_NAME,
    storeName = DEFAULT_INDEXED_DB_STORE,
    indexedDBImpl = globalThis.indexedDB,
} = {}) => {
    const resolvedDatabaseName = typeof databaseName === 'string' ? databaseName.trim() : '',
        resolvedStoreName = typeof storeName === 'string' ? storeName.trim() : '';
    if (!indexedDBImpl || typeof indexedDBImpl.open !== 'function')
        throw new TypeError('IndexedDB is not available in this environment');
    if (!resolvedDatabaseName || !resolvedStoreName)
        throw new TypeError('IndexedDB databaseName and storeName must be non-empty strings');
    ...
    const runTransaction = async (mode, operation) => {
        ...
        return await new Promise((resolve, reject) => {
            ...
            let request,
                result,
                settled = false;
            const transaction = database.transaction(resolvedStoreName, mode),
                fail = (error) => {
    ...
    return {
        ...
        async getItem(key) {
            const value = await runTransaction(
                'readonly',
                // Execute the transaction fixture
...
export const snapshotPersistentProps = (props) =>
    JSON.parse(
        JSON.stringify(
            Object.fromEntries(
...
export const decodePersistedValue = async (raw, { version = 1, key = '', component = null, migrate = null } = {}) => {
    ...
    const targetVersion = Math.max(1, Number(version) || 1),
        parsed = typeof raw === 'string' ? JSON.parse(raw) : raw;
    if (!parsed || typeof parsed !== 'object' || !hasOwn(parsed, 'version') || !hasOwn(parsed, 'data'))
        throw new TypeError('[ACL] Persisted records must use a { version, data } envelope.');

    const fromVersion = Number(parsed.version);
    if (!Number.isInteger(fromVersion) || fromVersion < 1)
        throw new TypeError('[ACL] Persisted record versions must be positive integers.');
    let data = parsed.data;

    if (fromVersion !== targetVersion && typeof migrate === 'function')
