...
const isMinifiedModule = new URL(import.meta.url).pathname.endsWith('.min.js'),
    ...
    { appendUiNode, createUiButton, toCss } = await importLocalModule('./overlay-utils.js');
...
export const createScannerUi = (documentRef) => {
    // Create scanner ui
    const style = append(documentRef, documentRef.createDocumentFragment(), 'style', '', stateCss),
        toggle = action(documentRef, 'A11y Audit', '');
    ...
    const title = append(documentRef, header, 'h2', 'acl-a11y-scanner-title', 'ACL Accessibility Audit', styles.title),
        headerClose = action(documentRef, '×', 'acl-a11y-scanner-button acl-a11y-scanner-close');
    ...
    const summary = append(
            ...
            dialog,
            'p',
            'acl-a11y-scanner-summary',
            'Ready to scan active ACL components.',
            styles.summary,
        ),
        results = documentRef.createElement('div'),
        footer = documentRef.createElement('footer'),
        rescan = action(documentRef, 'Rescan'),
        footerClose = action(documentRef, 'Close');
...
export const renderScannerResult = (documentRef, nodes, result) => {
    ...
    if (!result.componentCount || (!result.violationCount && !result.errorCount)) {
        append(
            ...
            nodes.results,
            'p',
            'acl-a11y-scanner-empty',
            result.componentCount ? 'No accessibility findings were detected.' : 'No active ACL components were found.',
            styles.empty,
        );
        return;
    }
    result.components.forEach((component, index) => {
        ...
        append(
            ...
            'h3',
            '',
            `${index + 1}. <${component.tag}> - ${component.violations.length} finding${component.violations.length === 1 ? '' : 's'}`,
            styles.groupTitle,
        );
        if (component.error)
            append(documentRef, group, 'p', 'acl-a11y-scanner-error', `Audit failed: ${component.error}`, styles.error);
