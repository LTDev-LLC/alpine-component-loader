...
const moduleSuffix = new URL(import.meta.url).pathname.endsWith('.min.js') ? '.min.js' : '.js',
    ...
    { stableStringify } = await importLocalModule('./values.js');
...
export const findExternalStyle = (url) => {
    ...
    return (
        Array.from(document.head.querySelectorAll('link[rel="stylesheet"]')).find(
            ...
            (link) => link.href === normalized || link.getAttribute('href') === url,
        ) || null
    );
};
...
export const findExternalScript = (url) => {
    ...
    return (
        Array.from(document.scripts).find(
            // Find the matching item
            (script) => script.src === normalized || script.getAttribute('src') === url,
        ) || null
    );
};
...
export const normalizeAssetDescriptor = (entry, type) => {
    ...
    return {
        ...
        ...(type === 'style' && entry.media ? { media: entry.media } : {}),
    };
};
...
export const normalizeAssetList = (entries, type) => {
    ...
    return (entries || [])
        .map(
            ...
            (entry) => normalizeAssetDescriptor(entry, type),
        )
        .filter((descriptor) => {
