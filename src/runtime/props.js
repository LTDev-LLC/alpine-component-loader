...
const moduleSuffix = new URL(import.meta.url).pathname.endsWith('.min.js') ? '.min.js' : '.js',
    ...
    { parseJson } = await importLocalModule('./values.js');
...
const normalizeSchema = (schema) => {
    ...
    return Object.fromEntries(
        Object.entries(schema).map(([key, value]) => {
            ...
            return [key, normalizeType(value)];
        }),
    );
};
    ...
    for (const [key, rawDefinition] of Object.entries(schema)) {
        ...
        const definition =
                rawDefinition && typeof rawDefinition === 'object' && !Array.isArray(rawDefinition)
                    ? rawDefinition
                    : { type: rawDefinition },
            expectedType = normalizeType(definition.type),
            nestedValue = value?.[key],
            nestedPath = path ? `${path}.${key}` : key;
        if (nestedValue == null) {
            if (definition.required) return `Missing required field "${nestedPath}".`;
            continue;
        }
        if (expectedType === String && typeof nestedValue !== 'string')
            return `Field "${nestedPath}" must be a string.`;
        if (expectedType === Number && typeof nestedValue !== 'number')
            return `Field "${nestedPath}" must be a number.`;
        if (expectedType === Boolean && typeof nestedValue !== 'boolean')
            return `Field "${nestedPath}" must be a boolean.`;
        if (expectedType === Array && !Array.isArray(nestedValue)) return `Field "${nestedPath}" must be an array.`;
        if (expectedType === Object && (typeof nestedValue !== 'object' || Array.isArray(nestedValue)))
            return `Field "${nestedPath}" must be an object.`;
...
export const parsePropDefinitions = (value) => {
    const invalid = Symbol('invalid-props'),
        parsed = typeof value === 'string' ? parseJson(value, invalid) : value || {};
    if (parsed === invalid || !parsed || typeof parsed !== 'object' || Array.isArray(parsed))
        throw new TypeError('[ACL] Prop definitions must be a JSON object.');
    return Object.fromEntries(
        Object.entries(parsed).map(
            // Transform the current item
            ([key, prop]) => [key, normalizePropDefinition(prop)],
        ),
    );
};
