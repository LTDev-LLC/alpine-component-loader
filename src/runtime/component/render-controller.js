...
const currentModuleUrl = new URL(import.meta.url),
    ...
    [
        { hasOwn, setBoundedMapEntry },
        { ACLLoadError, raceWithSignal },
        { cloneRuntimeValue, parseJson, stableStringify },
        { validateSchemaShape },
    ] = await Promise.all([
...
let assetsModulePromise = null,
    cachesModulePromise = null,
    renderingModulePromise = null;
...
export const withComponentRendering = (Base, { AlpineComponentLoader, settings, tagName, internalObservedAttrs }) => {
    return class extends Base {
                    ...
                    (descriptor) =>
                        new Promise((resolve, reject) => {
                            ...
                            const cleanup = () => {
                                ...
                                createError = () =>
                                    new ACLLoadError(`Failed to apply stylesheet: ${descriptor.url}`, {
                                ...
                                onError = () => finish(reject, createError()),
                                onAbort = () => finish(reject, createCancelError());
                            if (signal?.aborted) {
                                onAbort();
                                return;
                            }
                            ...
                            releaseCleanup = this._addCleanup(
        ...
        async _loadExternalDependencies(signal = null) {
            const [
                { applyAssetDescriptor, findExternalScript, findExternalStyle, normalizeAssetUrl },
                { scriptLoadCache, styleLoadPromiseCache },
            ] = await Promise.all([loadAssetsModule(), loadCachesModule()]);

            // Race shared global work against only this component's local load signal
            const awaitForComponent = (promise) =>
                raceWithSignal(
                    promise,
                    signal,
                    // Settle the abortable promise
                    () =>
                        new ACLLoadError('Component asset load was canceled.', {
                            code: 'ACL_LOAD_CANCELED',
                            phase: 'asset',
            ...
            await Promise.all(
                settings.externalCss.map(async (descriptor) => {
                    // Transform the current item
                    ...
                    if (!loadPromise) {
                        ...
                        loadPromise = new Promise((resolve, reject) => {
                            // Settle the asynchronous operation
                            let link = findExternalStyle(url),
                                timeoutId = null;
                            ...
                            const onError = () => {
                                ...
                                reject(
                                    new ACLLoadError(`Failed to load stylesheet: ${url}`, {
                                        code: 'ACL_ASSET_LOAD_FAILED',
                                        phase: 'asset',
                                    }),
                                );
                        ...
                        loadPromise.catch(() => {
                            // Handle the rejected operation
                            if (styleLoadPromiseCache.get(cacheKey) === loadPromise)
                                styleLoadPromiseCache.delete(cacheKey);
                        });
            ...
            for (const descriptor of settings.externalScripts) {
                ...
                if (!loadPromise) {
                    ...
                    loadPromise = new Promise((resolve, reject) => {
                        ...
                        if (
                            existing &&
                            (existing.dataset.aclLoaded === 'true' || existing.dataset.aclOwned !== 'true')
                        ) {
                            resolve();
                            return;
                        }
                        ...
                        const onError = () => {
                            ...
                            reject(
                                new ACLLoadError(`Failed to load script: ${url}`, {
                                    code: 'ACL_ASSET_LOAD_FAILED',
                                    phase: 'asset',
                                }),
                            );
            ...
            if (!this._hasRenderedContent) {
                ...
                const initialNodes = Array.from(this.childNodes).filter((node) => {
                    ...
                    if (
                        ...
                        (node.hasAttribute('data-acl-placeholder') || node.hasAttribute('data-acl-loading'))
                    ) {
                        node.remove();
                        return false;
                    }
        ...
        async _renderSafe(content, lightSlots) {
            ...
            if (!settings.stripStyles) {
                ...
                if (settings.shadow && settings.useConstructibleStyles && document.adoptedStyleSheets) {
                    ...
                    this._root.adoptedStyleSheets = [...(settings.sharedStyleSheets || []), ...(sheet ? [sheet] : [])];
            ...
            if (settings.executeScripts) {
                ...
                rootNode.querySelectorAll('script').forEach(
                    // Process the current item
                    (el) => el.remove(),
                );
            }
            ...
            if (!settings.shadow) {
                ...
                rootNode.querySelectorAll('slot').forEach((slotEl) => {
                    ...
                    // Preserve fallback slot content when no user nodes exist
        ...
        _setupEventForwarding() {
            ...
            settings.events.forward.forEach((eventRule) => {
                ...
                (this._forwardEventCleanups ||= []).push(
        ...
        _updateProp(name, value) {
            ...
            const type = configDef && configDef.type ? configDef.type : configDef,
                ...
                nullable = (configDef && configDef.nullable) === true,
                coerce = configDef && typeof configDef.coerce === 'function' ? configDef.coerce : null,
                validator = configDef && configDef.validator ? configDef.validator : null,
                configuredDefault = configDef && hasOwn(configDef, 'default') ? configDef.default : undefined,
                getDefaultValue = () =>
                    cloneRuntimeValue(
                        typeof configuredDefault === 'function' ? configuredDefault.call(this) : configuredDefault,
                    ),
            ...
            if (nullable && value === 'null') {
                ...
                if (
                    (type === Array && !Array.isArray(parsedValue)) ||
                    (type === Object &&
                        (typeof parsedValue !== 'object' || Array.isArray(parsedValue) || parsedValue === null))
                ) {
                    this._reportPropIssue(
                        `[ACL] Attribute "${name}" does not match its declared type on <${tagName}>.`,
                    );
                    parsedValue = type === Array ? [] : {};
                }
            } else {
            ...
            if (options && Array.isArray(options) && !options.includes(parsedValue)) {
                this._reportPropIssue(
                    `[ACL] Value "${parsedValue}" is not a valid option for prop "${name}" on <${tagName}>. Allowed: ${options.join(', ')}.`,
                );
            ...
            if (schema && type === Object && typeof parsedValue === 'object' && parsedValue !== null) {
                ...
                if (schemaError) {
                    this._reportPropIssue(
                        `[ACL] Schema validation failed for prop "${name}" on <${tagName}>: ${schemaError}`,
                    );
                    if (configuredDefault !== undefined) this.$props[name] = getDefaultValue();
                    else if (this.$props[name] === undefined) this._applyTypeDefault(name, type);
                    return;
