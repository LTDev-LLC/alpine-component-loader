...
const moduleSuffix = new URL(import.meta.url).pathname.endsWith('.min.js') ? '.min.js' : '.js',
    ...
    [
        ...
        { ACLLoadError, delayWithSignal, getRetryAfterMs, raceWithSignal, toACLLoadError },
        { cloneRuntimeValue, mergeFetchOptions, parseJson, toSearchParamsDeep, toUrlValue },
        { dataFetchCache, getDataCacheKey, normalizeRequestForCache, parseBodyValue, pruneDataFetchCache },
        { readBooleanAttribute, readNumberAttribute },
    ] = await Promise.all([
    ...
    return class extends Base {
        ...
        async _resolveFetchValue(attrName, configVal) {
            ...
            const attrVal = this.getAttribute(attrName),
                context = this._getFetchContext();

            if (attrVal) {
                const trimmed = attrVal.trim(),
                    invalid = Symbol('invalid'),
                    parsed = parseJson(trimmed, invalid);
                if (parsed === invalid || (!Array.isArray(parsed) && (!parsed || typeof parsed !== 'object')))
                    AlpineComponentLoader._report(
                        'warn',
                        `[ACL] ${attrName} must be a JSON object or array for <${tagName}>.`,
                        null,
                        {
                            tagName,
                            phase: 'fetch',
        ...
        async _resolveDataUrl(url) {
            const keys = await this._resolveFetchValue('data-fetch-keys', settings.data.keys),
                params = await this._resolveFetchValue('data-fetch-params', settings.data.params);
        ...
        _getDataFetchTimeout() {
            const raw = this.getAttribute('data-fetch-timeout') ?? settings.data.timeout,
                timeout = Number(raw);
            return Number.isFinite(timeout) && timeout > 0 ? timeout : 30000;
        }

        _getDataFetchNumber(attrName, configValue, fallback) {
            const configured = Number(configValue),
                defaultValue = Number.isFinite(configured) && configured >= 0 ? configured : fallback;
            return readNumberAttribute(this, attrName, defaultValue, { min: 0 });
        }

        _getDataFetchCacheTtl() {
            return readNumberAttribute(this, 'data-fetch-cache-ttl', Number(settings.data.cacheTtl), { min: 0 });
        }
        ...
        _getDataCacheStrategy(skipCache = false) {
            ...
            return ['cache-first', 'network-first', 'stale-while-revalidate', 'no-store'].includes(strategy)
                ? strategy
                : 'cache-first';
        }
        ...
        async _buildDataFetchOptions() {
            ...
            if (rawBody != null && !['GET', 'HEAD'].includes(fetchOptions.method)) {
        ...
        async _parseDataResponse(response) {
            if (typeof settings.data.parser === 'function')
                return await settings.data.parser.call(this, response, this._getFetchContext());
            ...
            if (responseType === 'json') {
                if (!(
                    contentType.includes('application/json') || /application\/[a-z0-9.+-]+\+json\b/.test(contentType)
                ))
                    throw new ACLLoadError('Invalid JSON response', {
        ...
        async _fetchResponseWithRetry(finalUrl, fetchOptions, controller) {
            const retries = this._getDataFetchNumber('data-retries', settings.data.retries, 0),
                retryDelay = this._getDataFetchNumber('data-retry-delay', settings.data.retryDelay, 250),
                retryMaxDelay = Math.max(
                    retryDelay,
                    this._getDataFetchNumber('data-retry-max-delay', settings.data.retryMaxDelay, 30000),
                ),
                jitter = Math.max(
                    0,
                    Math.min(1, this._getDataFetchNumber('data-retry-jitter', settings.data.retryJitter, 0)),
                ),
                method = String(fetchOptions.method || 'GET').toUpperCase(),
                methodCanRetry =
                    ['GET', 'HEAD', 'OPTIONS', 'PUT', 'DELETE'].includes(method) ||
                    readBooleanAttribute(this, 'data-retry-unsafe-methods', settings.data.retryUnsafeMethods),
                transientStatuses = new Set([408, 425, 429, 500, 502, 503, 504]);
            let lastError;
            ...
            for (let attempt = 0; attempt <= retries; attempt++) {
                ...
                try {
                    const res = await fetch(finalUrl, mergeFetchOptions(fetchOptions, { signal: controller.signal }));
                    if (!res.ok) {
                        const retryable = transientStatuses.has(res.status) && methodCanRetry,
                            error = new ACLLoadError(`API Error: ${res.status}`, {
                                code: 'ACL_HTTP_ERROR',
                                phase: 'fetch',
                                status: res.status,
                                retryable,
                    ...
                    if (controller.signal.aborted || attempt >= retries || lastError.retryable === false)
                        throw lastError;
                    const exponentialDelay = Math.min(retryMaxDelay, retryDelay * 2 ** attempt),
                        jitterMultiplier = jitter ? 1 + (Math.random() * 2 - 1) * jitter : 1,
                        waitMs = lastError.retryAfterMs ?? Math.max(0, Math.round(exponentialDelay * jitterMultiplier));
        ...
        _createDataCacheEntry(cacheKey, finalUrl, fetchOptions, target, strategy, retainOverride = null) {
            const sharedController = new AbortController(),
                fetchTask = this._fetchResponseWithRetry(finalUrl, fetchOptions, sharedController),
                ttl = this._getDataFetchCacheTtl(),
                shouldRetain = retainOverride ?? (Number.isFinite(ttl) && ttl > 0),
                entry = {
                    promise: fetchTask,
            ...
            pruneDataFetchCache({ cacheMax: this._getDataFetchCacheMax() });
            fetchTask
                .catch(() => {
                ...
                .finally(() => {
                    ...
                    if (!entry.retain && dataFetchCache.get(cacheKey)?.promise === fetchTask)
        ...
        async _consumeDataEntry(cacheEntry, cacheKey, signal, target) {
            ...
            const assertValid = () => {
                if (cacheEntry.invalidated)
                    throw new ACLLoadError('Data cache entry was invalidated.', {
        ...
        async _clearDataCache() {
            if (this._aclDebug?.dataCacheKey && AlpineComponentLoader.clearDataCache(this._aclDebug.dataCacheKey))
                return true;
        ...
        async _fetchData(url, skipCache = false, options = {}) {
            ...
            const timeoutMs = this._getDataFetchTimeout(),
            ...
            try {
                const finalUrl = await this._resolveDataUrl(url),
                    fetchOptions = await this._buildDataFetchOptions();
                let explicitCacheKey = this.getAttribute('data-cache-key') ?? settings.data.cacheKey;
                if (typeof explicitCacheKey === 'function')
                    explicitCacheKey = await explicitCacheKey.call(this, this._getFetchContext());
                const cacheVariant = {
                        responseType: this._getDataResponseType(),
                        parser: typeof settings.data.parser === 'function' ? tagName : null,
                    },
                    cacheIdentity = await getDataCacheKey(finalUrl, fetchOptions, explicitCacheKey, cacheVariant),
                    cacheKey = cacheIdentity.key || `acl-no-store-${generation}`;
                ...
                if (this._aclDebug.dataCacheHit)
                    this._dispatchAcl('cachehit', {
                        phase: 'data',
                        url: finalUrl,
                        strategy,
                        target,
                ...
                if (strategy === 'no-store') {
                    ...
                    refreshEntry.promise
                        .then(async () => {
                            // Handle the resolved operation
                            if (this.isConnected && !signal.aborted)
                                await this._consumeDataEntry(refreshEntry, cacheKey, signal, target);
                            if (this.isConnected && !signal.aborted)
                                this._dispatchAcl('revalidated', {
                                    phase: 'data',
                                    url: finalUrl,
                                    strategy,
                                    target,
