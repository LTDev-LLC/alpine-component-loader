    ...
    if (Object.getPrototypeOf(value) === Object.prototype || Object.getPrototypeOf(value) === null)
        return Object.fromEntries(
            Object.entries(value).map(
                ...
                ([key, nested]) => [key, cloneRuntimeValue(nested)],
            ),
        );
    return value;
};
...
export const toCssString = (styleObj) =>
...
export const toUrlValue = (value) =>
    encodeURIComponent(typeof value === 'object' && value !== null ? JSON.stringify(value) : value);
...
export const toSearchParamsDeep = (obj) => {
    const params = new URLSearchParams(),
        // Recursively preserve object paths and repeat array keys in source order
        add = (key, value) => {
...
export const parseListAttribute = (value) => {
    if (!value) return [];
    const invalid = Symbol('invalid-list'),
        parsed = parseJson(value, invalid);
...
export const normalizeForwardEvents = (...eventLists) =>
    eventLists
        .flatMap(
...
export const getTemplateLoadKey = (source, settings = {}) =>
    `${settings?._templateCacheKey || ''}::${settings?.templateRevision ? `${settings.templateRevision}::` : ''}${source}`;
