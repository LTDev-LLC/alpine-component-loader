...
const moduleSuffix = new URL(import.meta.url).pathname.endsWith('.min.js') ? '.min.js' : '.js',
    ...
    [{ VALID_CACHE_STRATEGIES, VALID_RESPONSE_TYPES }, { mergeFetchOptions, parseJson }] = await Promise.all([
...
const attributesForReaction = (reaction) =>
    new Set(
        DATA_OPTION_DESCRIPTORS.filter(
            // Select matching items
            (descriptor) => descriptor.attribute && descriptor.reaction === reaction,
        ).map(
...
export const DATA_FETCH_ATTRIBUTES = attributesForReaction('fetch'),
    DATA_POLL_ATTRIBUTES = attributesForReaction('poll'),
    DATA_CACHE_BOUND_ATTRIBUTES = attributesForReaction('cache-bound');

export const INTERNAL_COMPONENT_ATTRIBUTES = new Set(
    DATA_OPTION_DESCRIPTORS.filter(
        // Select matching items
        (descriptor) => descriptor.attribute,
    ).map(
...
export const DEFAULT_DATA_OPTIONS = Object.freeze(
    Object.fromEntries(
        DATA_OPTION_DESCRIPTORS.filter(
            // Select matching items
            ...
            // Transform the current item
            (descriptor) => [descriptor.group, descriptor.default],
        ),
    ),
);
...
export const readNumberAttribute = (element, name, fallback, { min = 0, positive = false } = {}) => {
    ...
    const value = Number(raw),
        lowerBound = positive ? Number.MIN_VALUE : min;
...
const configuredValue = (descriptor, localConfig, globalConfig) => {
    const localData = localConfig?.data || {},
        globalData = globalConfig?.data || {};
    return descriptor.group
        ? (localData[descriptor.group] ?? globalData[descriptor.group] ?? descriptor.default)
        : (localConfig?.[descriptor.setting] ?? globalConfig?.[descriptor.setting] ?? descriptor.default);
};

// Produce the complete effective data settings object from descriptor-driven configuration
export const resolveDataOptionSettings = (localConfig = {}, globalConfig = {}) =>
    Object.fromEntries(
        DATA_OPTION_DESCRIPTORS.filter(
            // Select matching items
            (descriptor) => descriptor.group,
            ...
            return [descriptor.group, configuredValue(descriptor, localConfig, globalConfig)];
        }),
    );
...
export const validateDataOptionSettings = (settings, suffix = '') => {
    DATA_OPTION_DESCRIPTORS.forEach((descriptor) => {
        ...
        const name = descriptor.group ? `data.${descriptor.group}` : descriptor.setting,
            value = descriptor.group ? settings?.data?.[descriptor.group] : settings?.[descriptor.setting];
        if (value == null) return;
        if (descriptor.allowed && !descriptor.allowed.has(value))
            throw new TypeError(`[ACL] Unsupported ${name} value "${value}"${suffix}.`);
        const numeric = Number(value);
        if (descriptor.validate === 'nonNegative' && (!Number.isFinite(numeric) || numeric < 0))
            throw new TypeError(`[ACL] ${name} must be a non-negative finite number${suffix}.`);
        if (descriptor.validate === 'unitInterval' && (!Number.isFinite(numeric) || numeric < 0 || numeric > 1))
            throw new TypeError(`[ACL] ${name} must be between 0 and 1${suffix}.`);
        if (descriptor.validate === 'positiveInteger' && (!Number.isInteger(numeric) || numeric < 1))
            throw new TypeError(`[ACL] ${name} must be a positive integer${suffix}.`);
    });
};
...
const parseDeclarativeValue = (element, descriptor, fallback) => {
    ...
    if (descriptor.parse === 'number') {
        ...
        return readNumberAttribute(element, attribute, fallback, { min: positive ? 1 : 0 });
    }
