...
const isMinifiedModule = new URL(import.meta.url).pathname.endsWith('.min.js'),
    ...
    { createUiButton, createUiNode } = await importLocalModule('./overlay-utils.js');
...
const css = {
    ...
    body: {
        ...
        display: 'grid',
        gridTemplateRows:
            'auto auto minmax(88px, 1fr) minmax(76px, 0.7fr) minmax(90px, 0.8fr) auto minmax(110px, 1fr) auto minmax(84px, 0.7fr) auto minmax(84px, 0.7fr) auto minmax(84px, 0.7fr) auto',
        gap: '10px',
        flex: '1 1 auto',
        minHeight: 0,
        overflow: 'hidden',
...
export const ensureDebuggerPanel = () => {
    ...
    panel = node('aside', { style: css.panel });
    panel.id = 'acl-debug-panel';
    panel.dataset.aclDebugUi = 'true';
    const style = node('style', { text: stateCss }),
        header = node('div', {
    ...
    header.append(node('span', { text: 'ACL Debugger' }), close);
    const body = node('div', {
            className: 'acl-debug-panel-body',
            style: css.body,
        }),
        summary = node('div', { style: css.section }),
        filters = node('div', {
        ...
        meta = node('pre', { style: css.pre }),
        props = node('pre', { style: css.pre }),
    ...
    const scroll = button('Scroll to selected'),
        reload = button('Reload selected'),
        clear = button('Clear selected cache'),
        snapshot = button('Capture snapshot'),
        diff = button('Diff latest'),
        exportButton = button('Export diagnostics'),
        timelineLabel = node('div', {
