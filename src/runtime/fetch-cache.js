...
const moduleSuffix = new URL(import.meta.url).pathname.endsWith('.min.js') ? '.min.js' : '.js',
    ...
    [{ hasOwn }, { parseJson, stableStringify }] = await Promise.all([
...
export const getDataCacheKey = async (finalUrl, fetchOptions, explicitKey = null, variant = null) => {
    const request = normalizeRequestForCache(fetchOptions),
        body = normalizeBodyForCache(fetchOptions?.body);
    if (!body.cacheable && !explicitKey)
        return {
            cacheable: false,
            reason: body.reason,
            key: null,
