...
const moduleSuffix = new URL(import.meta.url).pathname.endsWith('.min.js') ? '.min.js' : '.js',
    ...
    [
        { HTMLElementBase },
        { INTERNAL_COMPONENT_ATTRIBUTES, readDeclarativeOptionSettings },
        { parsePropDefinitions },
        { parseJson, parseListAttribute, resolveWindowPath },
    ] = await Promise.all([
...
export const createDeclarativeLoader = (loaderClass) =>
    class AlpineDeclarativeLoader extends HTMLElementBase {
        ...
        async _load() {
            ...
            const declaredProps = parsePropDefinitions(this.getAttribute('acl-props') || '{}'),
                declarativeOptions = readDeclarativeOptionSettings(this, loaderClass.globalConfig),
                config = {
                    shadow: this.hasAttribute('shadow')
                        ? this.getAttribute('shadow') !== 'false'
                        : loaderClass.globalConfig.shadow,
                    ...
                    templateCacheStrategy:
                        this.getAttribute('template-cache-strategy') || loaderClass.globalConfig.templateCacheStrategy,
                    ...
                    events: { forward: parseListAttribute(this.getAttribute('forward-events')) },
            ...
            if (!customElements.get(tagName)) {
                ...
                for (const attribute of this.attributes) {
                    ...
                    if (value.startsWith('[') || value.startsWith('{'))
                        inferredType = value.startsWith('[') ? Array : Object;
            ...
            // Process each attribute
            for (const attribute of this.attributes) {
                if (!loaderOnlyAttributes.has(attribute.name))
                    realElement.setAttribute(attribute.name, attribute.value);
            }
