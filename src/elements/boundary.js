...
export const createErrorBoundary = () =>
    class ACLErrorBoundary extends HTMLElement {
        constructor() {
            super();
            Object.defineProperty(this, '__aclBoundary', { value: true });
            this._errors = [];
            this._failed = new Set();
            const root = this.attachShadow({ mode: 'open' }),
                content = document.createElement('slot'),
                fallback = document.createElement('slot'),
        ...
        get errors() {
            return this._errors.map((entry) => ({ ...entry }));
        }
        ...
        _captureError(event) {
            ...
            this.dispatchEvent(
                new CustomEvent('acl:boundary-error', {
                    bubbles: true,
                    composed: true,
                    detail: entry,
                }),
        ...
        reset() {
            ...
            this.setAttribute('data-acl-boundary-state', 'ready');
            this.dispatchEvent(
                new CustomEvent('acl:boundary-reset', {
                    bubbles: true,
                    composed: true,
                    detail: { errors: previous },
                }),
            );
        }

        async retry() {
            const components = [...this._failed].filter(
                    // Run this operation
                    (component) => component?.isConnected,
                ),
                detail = {
            ...
            this.dispatchEvent(
                new CustomEvent('acl:boundary-retry', {
                    bubbles: true,
                    composed: true,
                    detail,
                }),
            ...
            const results = await Promise.allSettled(
                components.map(
                    // Run this operation
                    (component) =>
                        typeof component.reload === 'function'
                            ? component.reload({ reason: 'boundary-retry' })
                            : Promise.reject(new TypeError('Failed boundary child cannot reload.')),
                ),
            );
            if (
                results.every((result) => result.status === 'fulfilled') &&
                components.every((component) => component._state === 'ready')
            )
                this.reset();
