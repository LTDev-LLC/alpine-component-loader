...
const DEFAULT_DEBOUNCE = 50,
    ...
    INTERACTIVE_CONTAINER_SELECTOR = 'button, a[href], [role="button"], [role="link"]',
    ARIA_VALUE_RULES = {
...
const selectorFor = (element, root) => {
    ...
    while (current && current !== root && parts.length < 5) {
        const siblings = current.parentElement
            ...
            : [];
        parts.unshift(
            `${current.localName}${siblings.length > 1 ? `:nth-of-type(${siblings.indexOf(current) + 1})` : ''}`,
        );
...
export const runBasicAccessibilityAudit = (root) => {
    ...
    const violations = [],
        query = (selector) => Array.from(root.querySelectorAll?.(selector) || []);

    // Audit common names and form relationships
    query('img:not([alt])').forEach(
    ...
    query('button, [role="button"], input[type="button"], input[type="submit"], input[type="reset"]')
        .filter(
            // Select matching items
            ...
            (element) =>
                violations.push(
                    violation(
                        ...
                        element,
                        root,
                        'Provide visible text, aria-label, or aria-labelledby.',
                    ),
                ),
        );
    query('input:not([type="hidden"]), select, textarea').forEach((element) => {
        ...
        const id = element.id,
            labelled =
                ...
                (id && root.querySelector?.(`label[for="${globalThis.CSS?.escape ? CSS.escape(id) : id}"]`));
        if (!labelled)
            violations.push(
                violation(
                    'form-label',
                    ...
                    element,
                    root,
                    'Associate the field with a label or accessible name.',
                ),
            );
    });
                ...
                (element) =>
                    violations.push(
                        violation(
                            ...
                            element,
                            root,
                            'Use an ID that is unique within the component root.',
                        ),
                    ),
            ),
    );
        ...
        for (const attribute of ['aria-labelledby', 'aria-describedby', 'aria-controls']) {
            ...
            for (const id of (element.getAttribute(attribute) || '').split(/\s+/).filter(Boolean)) {
                if (
                    ...
                    !root.querySelector?.(`#${globalThis.CSS?.escape ? CSS.escape(id) : id}`)
                )
                    violations.push(
                        violation(
                            'aria-reference',
                            ...
                            element,
                            root,
                            `Ensure ${attribute} references an existing ID.`,
                        ),
                    );
            }
        }
    ...
    query('h1, h2, h3, h4, h5, h6').forEach((element) => {
        ...
        if (previousLevel && level > previousLevel + 1)
            violations.push(violation('heading-order', 'moderate', element, root, 'Do not skip heading levels.'));
        previousLevel = level;
    });
            ...
            (element) =>
                violations.push(
                    violation(
                        ...
                        element,
                        root,
                        'Provide descriptive link text, aria-label, or aria-labelledby.',
                    ),
                ),
        );
            ...
            (element) =>
                violations.push(
                    violation(
                        ...
                        element,
                        root,
                        'Provide visible or programmatic text for every heading.',
                    ),
                ),
        );
    query('iframe')
        .filter(
            // Select matching items
            (element) => !hasExplicitAccessibleName(element),
        )
        .forEach(
            // Process the current item
            (element) =>
                violations.push(
                    violation(
                        ...
                        element,
                        root,
                        'Add a concise title or accessible name that describes the frame.',
                    ),
                ),
        );
    ...
    query('[tabindex]')
        .filter(
            // Select matching items
            (element) => isPotentiallyFocusable(element) && Number(element.getAttribute('tabindex')) > 0,
        )
        .forEach(
            // Process the current item
            (element) =>
                violations.push(
                    violation(
                        ...
                        element,
                        root,
                        'Use DOM order and tabindex="0" instead of a positive tabindex.',
                    ),
                ),
        );
            ...
            (element) =>
                violations.push(
                    violation(
                        ...
                        element,
                        root,
                        'Remove focusable content from aria-hidden regions or remove aria-hidden.',
                    ),
                ),
        );
    });
    query(FOCUSABLE_SELECTOR)
        .filter(isPotentiallyFocusable)
        .forEach((element) => {
            // Process the current item
            ...
            if (parentInteractive)
                violations.push(
                    violation(
                        ...
                        element,
                        root,
                        'Do not nest one interactive control inside another.',
                    ),
                );
        });

    // Require the structural labels expected by common semantic containers
    query('dialog, [role="dialog"], [role="alertdialog"]')
        .filter(
            // Select matching items
            (element) => !hasExplicitAccessibleName(element),
        )
            ...
            (element) =>
                violations.push(
                    violation(
                        ...
                        element,
                        root,
                        'Give the dialog an aria-label, aria-labelledby reference, or title.',
                    ),
                ),
        );
    query('fieldset')
        .filter(
            // Select matching items
            (element) => !directChild(element, 'legend'),
        )
        .forEach(
            // Process the current item
            (element) =>
                violations.push(
                    violation(
                        ...
                        element,
                        root,
                        'Add a legend as a direct child to describe the grouped controls.',
                    ),
                ),
        );
    query('details')
        .filter(
            // Select matching items
            (element) => element.firstElementChild?.localName !== 'summary',
        )
        .forEach(
            // Process the current item
            (element) =>
                violations.push(
                    violation(
                        ...
                        element,
                        root,
                        'Add a summary as the first child of the details element.',
                    ),
                ),
        );
    query('table')
        .filter(
            // Select matching items
            (element) => !['none', 'presentation'].includes(element.getAttribute('role')),
        )
        .forEach((element) => {
            // Process the current item
            const caption = directChild(element, 'caption');
            if (!caption?.textContent?.trim() && !hasExplicitAccessibleName(element))
                violations.push(
                    violation(
                        'table-name',
                        'moderate',
                        element,
                        root,
                        'Add a caption or accessible name that describes the table.',
                    ),
                );
            if (element.querySelector('td') && !element.querySelector('th'))
                violations.push(
                    violation(
                        'table-headers',
                        'serious',
                        element,
                        root,
                        'Associate data cells with row or column headers.',
                    ),
                );
        });
    query('[role="img"]')
        .filter(
            ...
            (element) =>
                !hasExplicitAccessibleName(element) &&
                !(element.localName === 'svg' && directChild(element, 'title')?.textContent?.trim()),
        )
        .forEach(
            // Process the current item
            (element) =>
                violations.push(
                    violation(
                        ...
                        element,
                        root,
                        'Give meaningful graphics an aria-label, aria-labelledby reference, or title.',
                    ),
                ),
        );
    ...
    Object.entries(ARIA_VALUE_RULES).forEach(([attribute, allowed]) => {
        ...
        query(`[${attribute}]`)
            .filter(
                // Select matching items
                (element) => !allowed.includes(element.getAttribute(attribute)?.trim().toLowerCase()),
            )
            .forEach(
                // Process the current item
                (element) =>
                    violations.push(
                        violation(
                            ...
                            element,
                            root,
                            `Use a valid ${attribute} value: ${allowed.join(', ')}.`,
                        ),
                    ),
            );
    });
    query('[lang]')
        .filter(
            // Select matching items
            (element) => !hasValidLanguageTag(element.getAttribute('lang')),
        )
        .forEach(
            // Process the current item
            (element) =>
                violations.push(
                    violation(
                        ...
                        element,
                        root,
                        'Use a valid BCP 47 language tag, such as en or en-US.',
                    ),
                ),
        );
    return violations;
...
const normalizeViolations = (value, root) =>
...
export const auditAccessibility = async (root, { auditor = null } = {}) => {
    ...
    const startedAt = performance.now(),
        violations = auditor
            ? normalizeViolations(await auditor(root, { basic: runBasicAccessibilityAudit }), root)
            : runBasicAccessibilityAudit(root);
    return {
        violations,
...
export const observeAccessibility = (
    loaderClass,
    { auditor = null, debounce = DEFAULT_DEBOUNCE, logFindings = false } = {},
) => {
    if (typeof document === 'undefined')
        throw new TypeError('[ACL A11y] Accessibility observation requires browser DOM APIs.');
    ...
    const auditElement = async (element) => {
        ...
        const root = element.shadowRoot || element,
            result = await auditAccessibility(root, { auditor });
        ...
        if (logFindings && result.violations.length) {
            globalThis.console?.warn?.(
                `[ACL A11y] Found ${result.violations.length} accessibility finding${result.violations.length === 1 ? '' : 's'} in <${element.localName}>.`,
                result.violations,
            );
        }
        return result;
        ...
        onReady = (event) => {
            const element =
                event.composedPath?.().find(
                    // Run the element operation
                    (node) => node?.hasAttribute?.('data-acl-component'),
                ) || event.target;
            schedule(element);
        };
