...
export const createBatchExporter = ({
    send,
    batchSize = 20,
    flushInterval = 5_000,
    maxQueue = 200,
    ...
    if (!Number.isFinite(flushInterval) || flushInterval < 0)
        throw new TypeError('flushInterval must be a non-negative finite number.');
    if (!Number.isInteger(retries) || retries < 0) throw new TypeError('retries must be a non-negative integer.');
    if (!Number.isFinite(retryDelay) || retryDelay < 0)
        throw new TypeError('retryDelay must be a non-negative finite number.');
    let queue = [],
        timer = null,
        flushing = null,
        disposed = false;

    const schedule = // Run this operation
            () => {
            ...
            async (records) => {
                ...
                for (let attempt = 0; attempt <= retries; attempt++) {
                    ...
                    try {
                        ...
                        lastError = error;
                        if (attempt < retries)
                            // Wait before retrying the failed batch
                            await new Promise((resolve) => setTimeout(resolve, retryDelay * 2 ** attempt));
                    }
                }
                throw lastError;
            },
        flush = // Run this operation
            async () => {
                if (flushing) return flushing;
                if (!queue.length) return;
                clearTimeout(timer);
                timer = null;
                flushing = // Run this operation
                    (async () => {
                        // Process while
                        while (queue.length) {
        ...
        onPageHide = // Run this operation
            () => {
                void flush();
            },
        listener = // Run this operation
            (record) => {
                if (disposed) return;
                queue.push(record);
                if (queue.length > maxQueue) queue.splice(0, queue.length - maxQueue);
...
export const createBeaconExporter = // Run this operation
    ({
        url,
        fetch: suppliedFetch = globalThis.fetch,
        navigator: suppliedNavigator = globalThis.navigator,
        headers = {},
        ...batchOptions
    } = {}) => {
        ...
        return createBatchExporter({
            ...
            send: async (records) => {
                ...
                if (canBeacon && suppliedNavigator.sendBeacon(url, new Blob([body], { type: 'application/json' })))
                    return;
...
export const createOpenTelemetryExporter = // Run this operation
    ({ tracer = null, meter = null, logger = null, maxSpans = 200 } = {}) => {
        const spans = new Map(),
            counter = meter?.createCounter?.('acl.runtime.records');

        const listener = // Run this operation
            (record) => {
                // Process try
                try {
                    counter?.add?.(1, {
                    ...
                    if (record.requestId && record.type.endsWith('start') && tracer?.startSpan) {
                        spans.set(
                            record.requestId,
                            tracer.startSpan(`acl.${record.type}`, {
                                attributes: {
                        ...
                        span?.setStatus?.({ code: record.severity === 'error' ? 2 : 1 });
                        span?.end?.();
        ...
        listener.dispose = // Run this operation
            () => {
                spans.forEach(
                    // Run this operation
                    (span) => span.end?.(),
...
export const createSentryExporter = // Run this operation
    ({ client } = {}) => {
        ...
        return (record) => {
            ...
            try {
                if (record.severity === 'error')
                    client.captureMessage?.(record.detail?.message || record.type, {
                        level: 'error',
                        tags: {
                            // Configure this value
...
export const connectExporter = // Run this operation
    (loader, exporter) => {
