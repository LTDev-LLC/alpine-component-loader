...
const isMinifiedModule = new URL(import.meta.url).pathname.endsWith('.min.js'),
    ...
    [{ default: AlpineComponentLoader, createLoader }, { htmlToFragment }] = await Promise.all([
    ...
    return new Promise((resolve, reject) => {
        ...
        const cleanup = () => {
            ...
            matches = () => {
                ...
                return state === 'error'
                    ? element._state === 'idle' && Boolean(element.$props?.$error)
        ...
        timer = setTimeout(
            ...
            () => finish(reject, new Error(`[ACL Testing] Timed out waiting for state "${state}".`)),
            timeout,
        );
    });
};
...
const appendSlotValue = (element, name, value, settings) => {
    ...
    values.forEach((item) => {
        ...
        else if (item instanceof Node) nodes = [item];
        else nodes = [document.createTextNode(String(item ?? ''))];
        nodes.forEach((node) => {
            // Process the current item
...
export const mountComponent = async ({
    ...
    tagName = `acl-test-${++testTagSequence}`,
    template = '',
    options = {},
    attributes = {},
    properties = {},
    ...
    if (typeof document === 'undefined' || !container)
        throw new TypeError('[ACL Testing] mountComponent() requires browser DOM APIs and a container.');
        ...
        waitFor(name, { timeout = 2000 } = {}) {
            ...
            return new Promise((resolve, reject) => {
                ...
                waiter.timer = setTimeout(() => {
                    // Run the scheduled delayed task
                    const queue = (waiters.get(name) || []).filter(
...
const matchesRoute = (matcher, request) => {
    ...
    return typeof matcher === 'string'
        ...
        : matcher instanceof RegExp
          ? ((matcher.lastIndex = 0), matcher.test(request.url))
          : typeof matcher === 'function' && matcher(request);
};
...
export const installFetchMock = (routes = [], { target = globalThis } = {}) => {
    ...
    target.fetch = async (input, init = {}) => {
        ...
        const route = activeRoutes.find(
            // Find the matching item
            (candidate) =>
                (!candidate.method || candidate.method.toUpperCase() === request.method) &&
                matchesRoute(candidate.match ?? candidate.url, record),
        );
        ...
        if (route.delay) {
            await new Promise((resolve, reject) => {
                ...
                const finish =
                    ...
                    timer = setTimeout(finish(resolve), route.delay),
                    onAbort = () => {
...
export const assertLifecycleSequence = (records, expected, { exact = false } = {}) => {
    ...
    const actual = Array.from(records || [], (record) => record?.type || record?.name || String(record)),
        wanted = Array.from(expected || [], String);
    if (exact) {
        // Run this operation
        if (actual.length !== wanted.length || actual.some((name, index) => name !== wanted[index]))
            throw new Error(
                `[ACL Testing] Expected lifecycle sequence ${wanted.join(' -> ')}, received ${actual.join(' -> ')}.`,
            );
        return actual;
    }
    let cursor = 0;
    ...
    throw new Error(
        `[ACL Testing] Missing ordered lifecycle sequence ${wanted.join(' -> ')} in ${actual.join(' -> ')}.`,
    );
};

export const createACLTestHarness = ({
    loader = null,
    loaderOptions = {},
    container = typeof document === 'undefined' ? null : document.body,
    disposeLoader = loader == null,
} = {}) => {
    // Own mounts mocks recorders and an isolated loader so runner cleanup is deterministic
    let activeLoader = loader || createLoader(loaderOptions),
        cleaned = false;
    ...
    const assertActive = () => {
        ...
        cleanupResources = async () => {
            // Run this operation
            await Promise.allSettled([...mounts].map((mount) => mount.unmount()));
            mounts.clear();
            // Run this operation
