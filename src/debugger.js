...
const isMinifiedModule = new URL(import.meta.url).pathname.endsWith('.min.js'),
    ...
    [{ pushBoundedRecord }, { ACLLoadError }] = await Promise.all([
...
const MAX_CONTENT_LENGTH = 6000,
    ...
    VIRTUAL_LIST_THRESHOLD = 200,
    VIRTUAL_LIST_ROW_HEIGHT = 28,
    SENSITIVE_KEY_PATTERN = /(?:authorization|cookie|password|passwd|secret|token|api[-_]?key|credential)/i,
    ACL_EVENT_TYPES = [
        'acl:loadstart',
        'acl:loadend',
        'acl:error',
...
export const redactDiagnostics = (value, key = '', seen = new WeakSet()) => {
    ...
    for (const [childKey, childValue] of Object.entries(value))
        result[childKey] = redactDiagnostics(childValue, childKey, seen);
    return result;
};
...
const getElementContentSnapshot = (el) => {
    ...
    const attrs = el
            ...
            .join(' '),
        hostLine = `<${el.localName}${attrs ? ` ${attrs}` : ''}>`,
        lightDom = el.innerHTML.trim(),
        shadowDom = el.shadowRoot?.innerHTML?.trim() || '',
        renderedText = (el.shadowRoot || el).textContent.replace(/\s+/g, ' ').trim();
...
const createDebuggerState = (loaderClass, ensurePanel) => {
    const tooltip = ensureTooltip(),
        overlayContainer = ensureOverlay(),
        panel = ensurePanel();
        ...
        if (selectedComponentId) {
            selectedElement =
                components.find(
                    ...
                    (el) => getComponentId(el) === selectedComponentId,
                ) || null;
    ...
    const getSelectedElement = () => {
        ...
        if (!selectedElement && selectedComponentId)
            selectedElement =
                components.find(
                    // Find the matching item
                    (el) => getComponentId(el) === selectedComponentId,
    ...
    const onMouseMove = (e) => {
        ...
        mouseX = e.clientX;
        mouseY = e.clientY;
        const nextHoveredElement = (e.composedPath() || []).find(
            // Find the matching item
    ...
    const updateOverlays = () => {
        ...
        components.forEach((el) => {
            ...
            box.style.borderColor = isSelected
                ? debuggerCss.overlayBoxesColors.selected
                : debuggerCss.overlayBoxesColors[
                      el.$props?.$lastUpdated && Date.now() - el.$props.$lastUpdated < 1000 ? 'update' : 'default'
                  ];
            box.style.borderWidth = isSelected ? '4px' : '3px';
            box.style.boxShadow = isSelected
                ? '0 0 0 4px rgba(56,189,248,0.28), 0 0 24px rgba(14,165,233,0.55)'
                : 'none';
    ...
    const updateTooltip = () => {
        ...
        const debug = hoveredElement._aclDebug || {},
            dataInfo =
                debug.dataUrl && typeof loaderClass.getDataCacheInfo === 'function'
                    ? loaderClass.getDataCacheInfo(debug.dataUrl)
                    : null;
    ...
    const renderComponentList = () => {
        ...
        const selected = getSelectedElement(),
            query = panel._nodes.search.value.trim().toLowerCase(),
            status = panel._nodes.status.value,
            listedComponents = components.filter((element) => {
                // Select matching items
                const debug = element._aclDebug || {},
                    matchesQuery =
                        !query ||
                        [element.localName, element._state, debug.source, element.$props?.$error].some(
        ...
        if (listKey === renderedListKey) {
            if (revealSelectedInList) {
                const button = selectedComponentId
                    ? panel._nodes.list.querySelector(`button[data-acl-debug-id="${selectedComponentId}"]`)
                    : null;
        ...
        const listScrollTop = panel._nodes.list.scrollTop,
            virtualized = listedComponents.length > VIRTUAL_LIST_THRESHOLD,
            overscan = 8,
            visibleRows = Math.max(12, Math.ceil(panel._nodes.list.clientHeight / VIRTUAL_LIST_ROW_HEIGHT)),
            selectedIndex = selected ? listedComponents.indexOf(selected) : -1;
        let startIndex = virtualized ? Math.max(0, Math.floor(listScrollTop / VIRTUAL_LIST_ROW_HEIGHT) - overscan) : 0,
            endIndex = virtualized
                ? Math.min(listedComponents.length, startIndex + visibleRows + overscan * 2)
    ...
    const renderPanelDetails = () => {
        ...
        const selected = getSelectedElement(),
            dataInfo =
                typeof loaderClass.getDataCacheInfo === 'function'
                    ? loaderClass.getDataCacheInfo()
                    : {
                          size: 0,
                          keys: [],
                      },
        ...
        panel._nodes.timeline.textContent = timeline.length
            ? timeline
                  .slice(-30)
                  .map(
                      // Transform the current item
                      (entry) => `${entry.time} ${entry.type} <${entry.tag}>${entry.code ? ` ${entry.code}` : ''}`,
                  )
                  .join('\n')
            : 'No lifecycle events recorded.';
        panel._nodes.requests.textContent = requestActivity.length
            ? requestActivity
                  .slice(-30)
                  .map(
                      // Transform the current item
                      (entry) =>
                          `${entry.time} ${entry.type} ${entry.phase || ''} ${entry.source || ''}${entry.reason ? ` (${entry.reason})` : ''}`,
                  )
                  .join('\n')
        ...
        const debug = selected._aclDebug || {},
            selectedDataInfo =
                debug.dataUrl && typeof loaderClass.getDataCacheInfo === 'function'
                    ? loaderClass.getDataCacheInfo(debug.dataUrl)
                    : null;
        ...
        panel._nodes.a11y.textContent = selectedA11y
            ? selectedA11y.violations.length
                ? selectedA11y.violations
                      .map(
                          // Transform the current item
                          (item) => `${item.severity} ${item.rule} ${item.selector}\n${item.remediation}`,
                      )
                      .join('\n\n')
                : `No violations (${selectedA11y.duration.toFixed(1)}ms).`
            : 'No accessibility audit recorded.';
    ...
    const isDebuggerNode = (node) => {
        ...
        return Boolean(
            el &&
            (el.dataset?.aclDebugUi === 'true' ||
                el.hasAttribute?.('data-acl-debug-id') ||
                el === panel ||
                el === tooltip ||
                el === overlayContainer ||
                panel.contains(el) ||
                tooltip.contains(el) ||
                overlayContainer.contains(el)),
        );
    };
    ...
    const mutationObserver = new MutationObserver((mutations) => {
        ...
        const onlyDebuggerUiChanged = mutations.every(
            // Check every item
    ...
    const onLifecycleEvent = (event) => {
        ...
        if (event.type === 'acl:a11y' && target?.nodeType === 1)
            a11yResults.set(target, redactDiagnostics(event.detail || { violations: [] }));
        if (
            ['acl:loadstart', 'acl:loadend', 'acl:cachehit', 'acl:cacheevict', 'acl:revalidated'].includes(event.type)
        ) {
            pushBoundedRecord(
                requestActivity,
                {
    ...
    const onPanelListClick = (event) => {
        ...
        if (el) selectElement(el, { scroll: true });
    };

    panel._nodes.list.addEventListener('click', onPanelListClick);
    panel._nodes.list.addEventListener('scroll', onPanelListScroll, { passive: true });
    ...
    panel._nodes.diff.addEventListener('click', (event) => {
        ...
        event.stopPropagation();
        const selected = getActionTarget(),
            matching = snapshots.filter(
                // Select matching items
                (item) => item.componentId === selectedComponentId,
            );
        if (!selected || matching.length < 2) return;
        const before = matching[matching.length - 2],
            after = matching[matching.length - 1];
        panel._nodes.content.textContent = `${before.name} → ${after.name}\n\n${JSON.stringify(
            diffDiagnosticSnapshots(before.snapshot, after.snapshot),
            null,
            2,
        )}`;
    });
    ...
    static inject(loaderClass) {
        ...
        loaderClass.toggleDebug = function () {
            ...
            if (!this._debuggerStatePromise) {
                const loading = importDeferredLocalModule('./runtime/debugger-panel.js')
                    .then(({ ensureDebuggerPanel }) => {
