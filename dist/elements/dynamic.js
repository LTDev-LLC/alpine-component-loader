...
const moduleSuffix = new URL(import.meta.url).pathname.endsWith('.min.js') ? '.min.js' : '.js', importLocalModule = (specifier)=>import(/* @vite-ignore */ specifier.replace(/\.js$/, moduleSuffix)), [{ HTMLElementBase, validateCustomElementName }, { ACLLoadError, delayWithSignal, toACLLoadError }] = await Promise.all([
    importLocalModule('../runtime/config.js'),
    importLocalModule('../runtime/errors.js')
]);
...
const captureInlineStyles = (element, properties)=>Object.fromEntries([
        'transition',
        'willChange',
        ...properties
    ].map(// Transform the current item
    (property)=>[
            property,
            element.style[property]
        ]));
// Apply a compact inline style record to one transition target
const applyInlineStyles = (element, styles)=>Object.entries(styles).forEach(([property, value])=>{
...
const createTransitionValue = (properties, duration, easing)=>properties.map(// Transform the current item
    (property)=>`${property} ${duration}ms ${easing}`).join(', ');
// Create the dynamic switcher class bound to the loader's registry and global settings
export const createDynamicLoader = (loaderClass)=>class AlpineDynamicLoader extends HTMLElementBase {
        ...
        disconnectedCallback() {
            ...
            this._attrObserver = null;
            this._cache.forEach(// Process the current item
        ...
        _startAttributeObserver() {
            ...
            this._attrObserver = new MutationObserver(// Process observed DOM mutations
            (mutations)=>mutations.forEach((mutation)=>{
                    // Process the current item
                    if (mutation.type === 'attributes' && !internalAttributes.has(mutation.attributeName)) this._syncAttributeToChildren(mutation.attributeName);
                }));
        ...
        _pruneKeepAliveCache() {
            ...
            const evictions = Math.max(0, this._cache.size - Math.floor(max)), tags = this._cache.keys();
            // Iterate over the indexed values
            for(let index = 0; index < evictions; index++){
                const oldestTag = tags.next().value, oldest = this._cache.get(oldestTag);
        ...
        _swap(tag, generation, shouldRestoreFocus) {
            ...
            const keepAlive = this.hasAttribute('keep-alive'), current = this.firstElementChild;
            ...
            if (keepAlive && this._cache.has(tag)) {
                ...
                if (element._savedScroll) {
                    ...
                    this._scrollTimer = setTimeout(()=>{
                        ...
                        this._scrollTimer = null;
                        if (generation === this._switchGeneration && element.isConnected) element.scrollTop = element._savedScroll;
                    }, 0);
                }
            } else {
            ...
            if (shouldRestoreFocus) {
                const focusTarget = element.matches?.('[autofocus],[data-acl-autofocus]') ? element : element.querySelector?.('[autofocus],[data-acl-autofocus],button,input,select,textarea,a[href],[tabindex]:not([tabindex="-1"])');
                focusTarget?.focus?.({
                    preventScroll: true
                });
            }
        ...
        async _switch(rawTag) {
            ...
            if (!customElements.get(tag)) {
                this._reportSwitchError(new ACLLoadError(`Dynamic component <${tag}> is not registered.`, {
            ...
            const transitionController = new AbortController(), transitionSignal = transitionController.signal;
            this._transitionController = transitionController;
            this.setAttribute('aria-busy', 'true');
            const activeElement = document.activeElement, shouldRestoreFocus = Boolean(activeElement && this.contains(activeElement)), requested = this.getAttribute('transition') || loaderClass.globalConfig.dynamicTransition || 'auto', reducedMotion = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches === true, transition = reducedMotion ? 'none' : requested, useViewTransition = (transition === 'view' || transition === 'auto') && typeof document.startViewTransition === 'function', duration = Math.max(0, Number(this.getAttribute('transition-duration') ?? loaderClass.globalConfig.transitionDuration) || 0), preset = getTransitionPreset(transition), // Swap the active dynamic component while retaining focus state
            ...
            if (transition === 'none') {
                ...
                const current = this.firstElementChild, previous = current ? captureInlineStyles(current, preset.properties) : null;
                if (current) {
                ...
                if (incoming) {
                    ...
                    await new Promise((resolve)=>{
                        ...
                        transitionSignal.addEventListener('abort', finish, {
                            once: true
                        });
        ...
        _reportSwitchError(error, tag) {
            ...
            this.setAttribute('aria-busy', 'false');
            this.dispatchEvent(new CustomEvent('acl:error', {
                bubbles: true,
                composed: true,
                detail: {
                    component: this,
