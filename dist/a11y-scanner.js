...
const isMinifiedModule = new URL(import.meta.url).pathname.endsWith('.min.js'), resolveLocalModule = (specifier)=>isMinifiedModule ? specifier.replace(/\.js$/, '.min.js') : specifier, importLocalModule = (specifier)=>import(/* @vite-ignore */ resolveLocalModule(specifier)), importDeferredLocalModule = (specifier, retry = 0)=>import(/* @vite-ignore */ `${resolveLocalModule(specifier)}${retry ? `?acl-retry=${retry}` : ''}`), [{ auditAccessibility }, { restoreOverlayFocus, trapOverlayFocus }, { ACLLoadError }] = await Promise.all([
...
const DEFAULT_CONCURRENCY = 4, DEFAULT_BUTTON_POSITION = {
    bottom: 20,
    right: 20,
    gap: 8,
...
const collectActiveComponents = (root)=>{
    const components = [], seen = new Set(), add = (element)=>{
        // Add
        if (element?.isConnected && element.matches?.('[data-acl-component]') && !element.closest?.('[data-acl-a11y-scanner-ui]') && !seen.has(element)) {
            seen.add(element);
...
const dispatchResult = (element, result)=>element.dispatchEvent(new CustomEvent('acl:a11y', {
        bubbles: true,
        composed: true,
        detail: {
...
const scanComponents = async (root, { auditor, concurrency })=>{
    const startedAt = now(), elements = collectActiveComponents(root), components = new Array(elements.length);
    let cursor = 0;
    const worker = async ()=>{
        // Run the worker operation
        while(cursor < elements.length){
            const index = cursor++, element = elements[index];
            ...
            try {
                const result = await auditAccessibility(element.shadowRoot || element, {
                    auditor
                });
                components[index] = {
                    element,
    ...
    return {
        ...
        violationCount: components.reduce(// Accumulate the current item
        (total, item)=>total + item.violations.length, 0),
        errorCount: components.reduce(// Accumulate the current item
        (total, item)=>total + Number(Boolean(item.error)), 0),
        components
    };
};
// Mount one scanner controller per document
export const mountAccessibilityScanner = (options = {})=>{
    if (typeof document === 'undefined' || typeof window === 'undefined') throw new TypeError('[ACL A11y Scanner] Mounting requires browser DOM APIs.');
    if (!options || typeof options !== 'object' || Array.isArray(options)) throw new TypeError('[ACL A11y Scanner] mount() expects an options object.');
    const root = options.root || document, documentRef = root.nodeType === 9 ? root : root.ownerDocument;
    if (!documentRef || typeof root.querySelectorAll !== 'function') throw new TypeError('[ACL A11y Scanner] root must be a Document, Element, or open ShadowRoot.');
    if (mountedScanners.has(documentRef)) throw new Error('[ACL A11y Scanner] A scanner is already mounted in this document.');
    const requestedConcurrency = Number(options.concurrency ?? DEFAULT_CONCURRENCY);
    if (!Number.isFinite(requestedConcurrency) || requestedConcurrency < 1) throw new TypeError('[ACL A11y Scanner] concurrency must be a positive finite number.');
    if (options.auditor != null && typeof options.auditor !== 'function') throw new TypeError('[ACL A11y Scanner] auditor must be a function or null.');
    if (options.button != null && (typeof options.button !== 'object' || Array.isArray(options.button))) throw new TypeError('[ACL A11y Scanner] button must be an options object.');
    const view = documentRef.defaultView || window, concurrency = Math.max(1, Math.floor(requestedConcurrency)), buttonOptions = {
        ...DEFAULT_BUTTON_POSITION,
        ...options.button || {}
    };
    let nodes = null, dialogModule = null, destroyed = false, open = false, previousFocus = null, previousBodyOverflow = '', latestResult = null, scanPromise = null, mountPromise = null, dialogImportAttempt = 0, resolveDomReady;
    const domReady = documentRef.body ? Promise.resolve() : new Promise((resolve)=>{
    ...
    const assertActive = ()=>{
        ...
        const rect = companion.getBoundingClientRect(), gap = Number(buttonOptions.gap) || 0, viewportWidth = documentRef.documentElement?.clientWidth || view.innerWidth, viewportHeight = documentRef.documentElement?.clientHeight || view.innerHeight;
        ...
        const attempt = dialogImportAttempt++, mounting = domReady.then(async ()=>{
            ...
            try {
                ...
                if (destroyed) return null;
                if (error instanceof ACLLoadError && error.code === 'ACL_RUNTIME_MODULE_LOAD_FAILED') throw error;
            ...
            view.addEventListener('resize', positionToggle, {
                passive: true
            });
            return nodes;
        }).finally(()=>{
    ...
    else documentRef.addEventListener('DOMContentLoaded', onDomReady, {
        once: true
    });
    const controller = {
        scan,
...
const ACLA11yScanner = {
    mount: mountAccessibilityScanner
};
