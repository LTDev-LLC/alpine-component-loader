...
export const reloadChangedTemplates = async (sources, loader = AlpineComponentLoader)=>{
    ...
    const normalizedSources = new Set((Array.isArray(sources) ? sources : [
        sources
    ]).filter(Boolean).map(normalizeSource)), matchingDefinitions = loader.getRegisteredTags().map(// Transform the current item
    (tagName)=>loader.getDefinition(tagName)).filter(// Select matching items
    (definition)=>normalizedSources.has(normalizeSource(definition?.source))), reloadTargets = new Set();
    // Clear every matching definition before collecting its instances across shadow boundaries
    await Promise.all(matchingDefinitions.map(async (definition)=>{
        // Transform the current item
        await loader.clearTemplate(definition.source, definition.settings?._templateCacheKey);
        collectRoots(document).forEach(// Process the current item
        (root)=>root.querySelectorAll?.(definition.tagName).forEach(// Reload changed templates
            (element)=>reloadTargets.add(element)));
    }));
    const reloadResults = await Promise.allSettled(Array.from(reloadTargets, (element)=>{
        // Transform the current item
        ...
        tags: matchingDefinitions.map(// Transform the current item
        (definition)=>definition.tagName),
        reloaded: reloadResults.filter(// Select matching items
        (result)=>result.status === 'fulfilled').length,
        failed: reloadResults.filter(// Select matching items
        (result)=>result.status === 'rejected').length
    };
    window.dispatchEvent(new CustomEvent('acl:dev-reload', {
        detail
    }));
    return detail;
};
...
const prepareInlineTemplateUpdates = (templates, loader)=>{
    if (!Array.isArray(templates) || !templates.length) throw new TypeError('[ACL Dev] Inline template updates require a non-empty template list.');
    const seen = new Set();
    return templates.map((update)=>{
        const kind = update?.kind, name = String(update?.name || '').trim(), key = `${kind}:${name}`;
        if (![
            'component',
            'id'
        ...
        (definition)=>kind === 'component' ? definition?.tagName === name && definition.source === template : definition?.source === template || definition?.source === `#${name}`);
        if (!definitions.length) throw new TypeError(`[ACL Dev] Inline template "${name}" is not an active ACL definition.`);
        return {
            definitions,
            html: update.html,
            template
...
const reloadInlineTemplateRevision = async (message, loader)=>{
    ...
    if (payload?.revision !== message.revision || payload?.source !== message.source || !Array.isArray(payload.templates)) throw new TypeError('[ACL Dev] Inline template revision did not match its event.');
    const expected = new Set((message.templates || []).map(// Normalize one announced template identity
    (template)=>`${template?.kind}:${String(template?.name || '').trim()}`)), received = new Set(payload.templates.map(// Normalize one fetched template identity
    (template)=>`${template?.kind}:${String(template?.name || '').trim()}`));
    if (expected.size !== received.size || [
        ...expected
    ].some(// Require the fetched revision to contain exactly the announced templates
    (key)=>!received.has(key))) throw new TypeError('[ACL Dev] Inline template revision contents did not match its event.');
    const updates = prepareInlineTemplateUpdates(payload.templates, loader), replacements = updates.map(// Parse every inert replacement before changing any live template
    (update)=>({
            ...update,
    ...
    const sources = [
        ...new Set(replacements.flatMap(({ definitions })=>definitions.map(// Reload through each definition's original source identity
...
export const connectACLDevServer = ({ url = null, loader = AlpineComponentLoader, EventSourceImpl = globalThis.EventSource } = {})=>{
    if (typeof window === 'undefined' || typeof EventSourceImpl !== 'function') throw new TypeError('[ACL Dev] A browser-compatible EventSource implementation is required.');
    const eventSource = new EventSourceImpl(url || '/__acl_hmr/events');
    let closed = false, inlineQueue = Promise.resolve(), fallbackStarted = false;
    ...
    const onMessage = (event)=>{
        ...
        try {
            ...
            if ([
                'template-changed',
                'acl:template-changed'
            ].includes(message?.type)) {
                const sources = message.sources || message.source;
                void reloadChangedTemplates(sources, loader).then((result)=>{
            ...
            if ([
                'page-reload',
                'acl:page-reload'
            ].includes(message?.type)) fallbackReload();
        } catch (error) {
