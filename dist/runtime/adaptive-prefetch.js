...
const closestTarget = (node, root, selector)=>{
    ...
    const element = node?.nodeType === 1 ? node : node?.parentElement, target = element?.closest?.(selector);
...
const connectionSkipReason = (options)=>{
    ...
    if ([
        'slow-2g',
        '2g'
    ].includes(connection.effectiveType)) return 'constrained-network';
    return null;
};
export const createAdaptivePrefetchController = (loaderClass, supplied = {})=>{
    ...
    const options = {
        ...DEFAULTS,
        ...supplied || {}
    }, root = options.root || document, triggers = new Set(options.triggers || DEFAULTS.triggers), completed = new Set(), timers = new Map(), observed = new Set();
    let disconnected = false, intersectionObserver = null, mutationObserver = null, idleId = null;
    ...
    const resolveTokens = (target)=>{
        // Resolve tokens
        const raw = typeof target === 'string' ? target : target?.getAttribute?.('data-acl-prefetch') || target?.localName || '', tokens = String(raw).split(/[\s,]+/).map(// Transform the current item
        (value)=>value.trim().toLowerCase()).filter(Boolean), tags = [];
        // Process each token
    ...
    const prefetch = async (target)=>{
        ...
        const tags = Array.isArray(target) ? [
            ...new Set(target.flatMap(resolveTokens))
        ] : resolveTokens(target), pending = tags.filter(// Select matching items
        (tag)=>!completed.has(tag));
        if (!pending.length) return {};
        pending.forEach(// Process the current item
        (tag)=>completed.add(tag));
        dispatch('prefetchstart', {
            tags: pending
        });
        const results = await loaderClass.prefetchGraph(pending, {
        ...
        dispatch('prefetchend', {
            tags: pending,
            fulfilled: Object.values(results).filter(// Select matching items
            (result)=>result.status === 'fulfilled').length,
            rejected: Object.values(results).filter(// Select matching items
    ...
    const releaseElement = (element)=>{
        ...
        observed.forEach(// Release one observed descendant
        (element)=>{
            if (element === node || node?.contains?.(element)) releaseElement(element);
        });
        timers.forEach(// Release one delayed hover descendant
        (_, element)=>{
            if (element === node || node?.contains?.(element)) releaseElement(element);
        });
    ...
    const onPointerOver = (event)=>{
        ...
        const target = closestTarget(event.target, root, options.selector), timer = target && timers.get(target);
    ...
    if ((triggers.has('viewport') || triggers.has('visible')) && typeof IntersectionObserver !== 'undefined') {
        intersectionObserver = new IntersectionObserver(// Process intersection changes
    ...
    if (typeof MutationObserver !== 'undefined') {
        mutationObserver = new MutationObserver(// Process observed DOM mutations
        (records)=>{
            records.forEach(// Release removed mutation subtrees before considering reinsertions
            (record)=>record.removedNodes.forEach(releaseSubtree));
            records.forEach(// Observe matching targets from every added mutation subtree
    ...
    if (triggers.has('idle')) {
        ...
        if (typeof requestIdleCallback === 'function') idleId = requestIdleCallback(run, {
            timeout: 2000
        });
