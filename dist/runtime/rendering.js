...
const moduleSuffix = new URL(import.meta.url).pathname.endsWith('.min.js') ? '.min.js' : '.js', importLocalModule = (specifier)=>import(/* @vite-ignore */ specifier.replace(/\.js$/, moduleSuffix)), { ACLLoadError } = await importLocalModule('./errors.js');
const URL_ATTRIBUTES = new Set([
    ...
    'poster',
    'data'
]), EXECUTABLE_PROTOCOL = /^(?:javascript|vbscript):/i, EXECUTABLE_DATA = /^data\s*:\s*(?:text\/html|application\/xhtml\+xml|image\/svg\+xml)/i, parsedFragmentCaches = new WeakMap();
...
export const htmlToFragment = (html, settings = {})=>{
    ...
    try {
        ...
        if (cause?.name === 'TypeError') throw new ACLLoadError('A Trusted Types policy is required to parse component HTML.', {
    ...
    [
        ...doc.head.childNodes,
        ...doc.body.childNodes
    ].forEach(// Process the current item
...
export const cloneParsedFragment = (html, settings = {})=>{
    if (settings?.security?.trustedTypesPolicy || !settings || typeof settings !== 'object') return htmlToFragment(html, settings);
    const cache = parsedFragmentCaches.get(settings) || [], source = String(html), cachedIndex = cache.findIndex(// Find a matching parsed source within the tiny definition cache
...
export const sanitizeNodeTree = (rootNode, settings = {}, context = {})=>{
    collectElements(rootNode).forEach((element)=>{
        ...
        if (localName === 'script' || localName === 'base' || localName === 'meta' && element.getAttribute('http-equiv')?.trim().toLowerCase() === 'refresh') {
            element.remove();
            return;
        }
        Array.from(element.attributes).forEach((attribute)=>{
            ...
            if (name === 'srcset') {
                const candidates = value.split(',').map(// Transform the current item
                (candidate)=>candidate.trim().split(/\s+/)[0]).filter(Boolean);
                if (candidates.some(// Check the current item
                (candidate)=>!policyAllowsUrl(candidate, policyContext))) element.removeAttribute(attribute.name);
            }
        });
    });
    return rootNode;
...
const splitSelectorList = (selectorText)=>{
    const selectors = [];
    let current = '', depth = 0, quote = null, escaped = false;
...
export const scopeSelectorText = (selectorText, scopeSelector)=>splitSelectorList(selectorText).map((selector)=>{
        // Transform the current item
        let scoped = selector.replace(/:host\s*\(([^)]+)\)/g, `${scopeSelector}$1`).replace(/:host\b/g, scopeSelector).replace(/:scope\b/g, scopeSelector).replace(/^\s*(?:html|body|:root)\b/, scopeSelector);
...
export const scopeLightDomCss = (cssText, scopeSelector)=>{
    ...
    try {
        ...
        const rewriteRules = (rules)=>Array.from(rules || []).forEach((rule)=>{
                // Process the current item
                ...
                if (rule.cssRules && !(typeof CSSKeyframesRule !== 'undefined' && rule instanceof CSSKeyframesRule)) rewriteRules(rule.cssRules);
            });
        rewriteRules(sheet?.cssRules);
        return Array.from(sheet?.cssRules || []).map(// Transform the current item
