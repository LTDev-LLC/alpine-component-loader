...
const moduleSuffix = new URL(import.meta.url).pathname.endsWith('.min.js') ? '.min.js' : '.js', importLocalModule = (specifier)=>import(/* @vite-ignore */ specifier.replace(/\.js$/, moduleSuffix)), { cloneRuntimeValue } = await importLocalModule('./values.js');
// Snapshot only clonable application props and omit loader helper state
const publicProps = (props)=>Object.fromEntries(Object.entries(props || {}).filter(// Select matching items
    ([name, value])=>!name.startsWith('$') && typeof value !== 'function').map(// Transform the current item
    ([name, value])=>[
            name,
...
const preserveKey = (element, root)=>{
    ...
    if (element.getAttribute('name') && root.querySelectorAll(`[name="${selectorEscape(element.getAttribute('name'))}"]`).length === 1) return `name:${element.getAttribute('name')}`;
    return `path:${elementPath(element, root)}`;
};
const resolveKey = (key, root)=>{
    // Resolve key
    const [kind, ...parts] = key.split(':'), value = parts.join(':');
...
export const captureReloadState = (element, root, props)=>{
    const controls = Array.from(root.querySelectorAll('input, textarea, select')).map(// Transform the current item
    (control)=>({
        ...
        })), scroll = Array.from(root.querySelectorAll('*')).filter(// Select matching items
    (node)=>node.scrollTop || node.scrollLeft).map(// Transform the current item
    (node)=>({
            key: preserveKey(node, root),
            top: node.scrollTop,
            left: node.scrollLeft
        })), active = root.activeElement || (root.contains?.(document.activeElement) ? document.activeElement : null), alpine = globalThis.Alpine ? Array.from(root.querySelectorAll('[x-data]')).map(// Snapshot public local Alpine state using the same stable DOM keys as controls
    (node)=>({
            key: preserveKey(node, root),
...
export const restoreReloadDomState = (element, root, snapshot)=>{
    ...
    for (const state of snapshot?.alpine || []){
        const node = resolveKey(state.key, root), data = node && (node._x_dataStack?.[0] || globalThis.Alpine?.$data?.(node));
        if (!data) continue;
    ...
    for (const state of snapshot?.controls || []){
        ...
        if (state.selectedIndex !== undefined && 'selectedIndex' in control) control.selectedIndex = state.selectedIndex;
