...
const moduleSuffix = new URL(import.meta.url).pathname.endsWith('.min.js') ? '.min.js' : '.js', importLocalModule = (specifier)=>import(/* @vite-ignore */ specifier.replace(/\.js$/, moduleSuffix)), [{ DATA_CACHE_BOUND_ATTRIBUTES, DATA_FETCH_ATTRIBUTES, DATA_POLL_ATTRIBUTES }, { applyLifecycleState }] = await Promise.all([
...
export const withComponentLifecycle = (Base, { AlpineComponentLoader, settings, tagName, contentSource, helpers, observedAttrs })=>{
    class Component extends Base {
        constructor(){
            ...
            let _$props = {
                ...
                ...helpers(this),
                ...Object.fromEntries(Object.keys(settings.attributes || {}).map(// Transform the current item
                (name)=>[
                        name,
                        undefined
                    ]))
            ...
            const declarativeShadowTemplate = settings.shadow && !this.shadowRoot ? Array.from(this.children || []).find(// Find the matching item
            (child)=>child.localName === 'template' && child.hasAttribute('data-acl-ssr-shadow') && child.getAttribute('shadowrootmode') === 'open') : null;
            this._root = settings.shadow ? this.shadowRoot || this.attachShadow({
                mode: 'open'
            }) : this;
            ...
            const initialDataElement = Array.from(this.children || []).find(// Run this operation
            (child)=>child.localName === 'script' && child.hasAttribute('data-acl-ssr-data'));
            if (initialDataElement) {
                // Run this operation
                try {
                    const initialData = JSON.parse(initialDataElement.textContent || 'null'), initialTarget = settings.data.target || '$data';
            ...
            this._ssrHydration = Boolean(this.hasAttribute('data-acl-ssr') && this._root.childNodes.length && this.getAttribute('data-acl-revision') === String(settings.templateRevision || ''));
        ...
        _installAlpinePropsScope() {
            ...
            const host = this, scope = {}, hadOwnDataStack = Object.prototype.hasOwnProperty.call(this, '_x_dataStack');
            ...
            const removeScope = window.Alpine.addScopeToNode(this, scope), cleanup = ()=>{
        ...
        attributeChangedCallback(name, oldVal, newVal) {
            ...
            if (DATA_FETCH_ATTRIBUTES.has(name)) {
                ...
                AlpineComponentLoader._pruneDataFetchCache({
        ...
        _startPolling() {
            ...
            this._pollTimer = setTimeout(async ()=>{
                ...
                if (src && this._state === 'ready' && !this.$props.$loading && !this._isPollingPaused()) await this._fetchData(src, true);
                // Schedule next tick
                this._startPolling();
            }, interval);
        }
        ...
        _attachPollingSignals() {
            ...
            if (this._getPollingPauseSetting('pause-polling-when-hidden', 'pauseWhenHidden')) {
                ...
                (this._pollSignalCleanups ||= []).push(// Capture the pushed value
            ...
            if (this._getPollingPauseSetting('pause-polling-when-offscreen', 'pauseWhenOffscreen')) {
                if ('IntersectionObserver' in window) {
                    ...
                    const observer = new IntersectionObserver((entries)=>{
                        ...
                        const wasIntersecting = this._pollIsIntersecting, isIntersecting = entries.some(// Check the current item
        ...
        _detachPollingSignals() {
            this._pollSignalCleanups?.forEach(// Process the current item
            (cleanup)=>cleanup());
            this._pollSignalCleanups = null;
            this._pollIntersectionObserver = null;
            this._pollIsIntersecting = true;
        ...
        _initHydrationTrigger(mode) {
            if (mode === 'visible') {
                ...
                const observer = new IntersectionObserver(// Run this operation
                (entries)=>{
                    if (entries.some(// Run this operation
                    (entry)=>entry.target === this && entry.isIntersecting)) this._beginHydration();
                }, {
                    rootMargin: '100px'
                });
                observer.observe(this);
            ...
            if (mode === 'interaction') {
                const start = ()=>this._beginHydration(), events = [
                    'pointerover',
                    'touchstart',
                    'focusin'
                ];
                events.forEach((event)=>this.addEventListener(event, start, {
                        // Configure this value
                        capture: true,
                        passive: true
                    }));
                (this._hydrationCleanups ||= []).push(()=>events.forEach((event)=>this.removeEventListener(event, start, {
                            capture: true
                        })));
                return;
            }
            if (mode === 'media') {
                const query = this.getAttribute('hydrate-media') || settings.hydrateMedia, media = window.matchMedia(query), change = ()=>{
        ...
        _initSlotObserver() {
            ...
            this._slotObserver = new MutationObserver(// Process observed DOM mutations
            (mutations)=>mutations.forEach((mutation)=>{
                    ...
                    mutation.addedNodes.forEach((node)=>{
                        ...
                        const slotName = (node.nodeType === Node.ELEMENT_NODE ? node.getAttribute('slot') : null) || 'default', container = this._root.querySelector(`div[data-acl-slot="${slotName}"]`);
    ...
    Object.entries(settings.attributes || {}).forEach(([name, definition])=>{
        ...
        Object.defineProperty(Component.prototype, name, {
            ...
            set (value) {
                const configDefinition = definition && definition.type ? definition : {}, type = configDefinition.type || definition;
