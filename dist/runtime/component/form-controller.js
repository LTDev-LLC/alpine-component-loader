...
const normalizeFormValue = (value)=>{
    if (value == null || typeof value === 'string' || typeof File !== 'undefined' && value instanceof File || typeof FormData !== 'undefined' && value instanceof FormData) return value;
    if (typeof value === 'object') return JSON.stringify(value);
    return String(value);
};
export const withComponentForm = (Base, { AlpineComponentLoader, settings, tagName })=>class extends Base {
        ...
        constructor(){
            ...
            try {
                ...
                AlpineComponentLoader._report('warn', `[ACL] Form internals are unavailable for <${tagName}>.`, error, {
                    tagName,
                    phase: 'form'
                });
            }
        ...
        attributeChangedCallback(name, oldValue, newValue) {
            ...
            if (oldValue !== newValue && [
                ...
                'disabled',
                'required'
            ].includes(name)) this._syncFormState();
        }
        // Run this operation
        async _initAlpine() {
            ...
            if (!this._formEffectInstalled && typeof window.Alpine?.effect === 'function') {
                ...
                const runner = window.Alpine.effect(// Run this operation
                ()=>{
                ...
                if (window.Alpine.release) this._addCleanup(// Run this operation
                ()=>{
                    this._formEffectInstalled = false;
        ...
        _syncFormState(captureDefault = false) {
            const form = settings.form, value = this.$props[form.value], state = form.state ? this.$props[form.state] : value, disabled = Boolean(this.hasAttribute('disabled') || form.disabled && this.$props[form.disabled]);
            ...
            if (this._formInternals) this._formInternals.setFormValue(disabled ? null : normalizeFormValue(value ?? ''), normalizeFormValue(state ?? value ?? ''));
            else {
        ...
        setFormValue(value, state = value) {
            ...
            if (this._formInternals) this._formInternals.setFormValue(normalizeFormValue(value), normalizeFormValue(state));
            else this._syncFormState();
        }
        // Run this operation
        setValidity(flags = {}, message = '', anchor = undefined) {
            if (!this._formInternals) {
                AlpineComponentLoader._report('warn', `[ACL] Native form validity is unavailable for <${tagName}>.`, null, {
