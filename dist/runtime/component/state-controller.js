...
}, importLocalModule = (specifier)=>import(/* @vite-ignore */ resolveLocalModule(specifier)), importDeferredLocalModule = (specifier)=>import(/* @vite-ignore */ resolveLocalModule(specifier)), [{ hasOwn }, { ACLLoadError, toACLLoadError }, { cloneRuntimeValue, toCssString }, { readNumberAttribute }, { createLifecycleEventDetail }] = await Promise.all([
...
export const withComponentState = (Base, { AlpineComponentLoader, settings, tagName, observedAttrs })=>{
    return class extends Base {
        ...
        _syncAllAttributes() {
            observedAttrs.forEach(// Process the current item
            (name)=>this._updateProp(name, this.getAttribute(name)));
        }
        async _releasePersistence(flush = true) {
            const dispose = this._persistenceDispose, persistence = this.$props.$persistence;
        ...
        async _initPersistence() {
            ...
            if (!mode) return;
            const { createIndexedDBPersistenceAdapter, createPersistenceEnvelope, decodePersistedValue, snapshotPersistentProps } = await loadRuntimeModule('../persistence.js');
            // Normalize identity and route all adapter failures through the component error boundary
            const key = this.getAttribute('persist-key') || settings.persistKey || `acl:${this.localName}:${this.id || 'default'}`, debounceMs = Math.max(0, parseInt(this.getAttribute('persist-debounce') || settings.persistDebounce || '250', 10) || 0), version = Math.max(1, readNumberAttribute(this, 'persist-version', Number(settings.persistVersion) || 1, {
                min: 1
            ...
            if (!adapter) {
                ...
                try {
                    ...
                    if (normalizedMode === 'indexeddb') adapter = AlpineComponentLoader._indexedDBPersistenceAdapter ||= createIndexedDBPersistenceAdapter();
                    else {
            ...
            if (!adapter || ![
                ...
                'setItem',
                'removeItem'
            ].every(// Check every item
            (method)=>typeof adapter[method] === 'function')) {
                reportPersistenceError(new TypeError('Persistence adapter must implement getItem, setItem, and removeItem.'), 'initialize');
                return;
            }
            // Debounce timer for persistence
            let _timer = null, pendingValue, pendingHasValue = false, pendingDeferred = null, writeChain = Promise.resolve(), disposed = false, disposePromise = null, persistRunner = null, onPageHide = null;
            ...
            const saveNow = async (value, hasValue = false)=>{
                ...
                const data = cloneRuntimeValue(hasValue ? value : getSnapshot()), operation = writeChain.catch(()=>{
            ...
            this.$props.$persistence = {
                ...
                $clear: async ()=>{
                    ...
                    const operation = writeChain.catch(()=>{
            ...
            const stored = await readData();
            if (stored && typeof stored === 'object') // Process each prop name
            {
                for(const propName in this.$props)if (hasOwn(stored, propName)) this.$props[propName] = cloneRuntimeValue(stored[propName]);
            }
        ...
        async _initAlpine() {
            ...
            return new Promise((resolve, reject)=>{
                ...
                cancel = (reason = 'Canceled')=>{
                    ...
                    settled = true;
                    cleanup();
                    reject(new ACLLoadError(`Alpine initialization ${String(reason).toLowerCase()}.`, {
        ...
        async _finishAlpineInit() {
            ...
            if (storeName) {
                ...
                if (store) {
                    ...
                    if (this.$props && typeof this.$props === 'object') Object.keys(this.$props).filter(// Select matching items
                    (key)=>store[key] === undefined).forEach(// Process the current item
                    (key)=>store[key] = this.$props[key]);
                    // Set $props to Alpine store
                    this.$props = store;
                } else {
                    AlpineComponentLoader._report('error', `[ACL] Store "${storeName}" not found. Falling back to local state.`, null, {
                        tagName,
                        phase: 'store'
                    });
            ...
            if (updateRunner && window.Alpine.release) this._addCleanup(// Register cleanup work
            ...
            if (this._root && this._root.children) Array.from(this._root.children).forEach((node)=>{
                // Process the current item
        ...
        _dispatchAcl(eventName, detail = {}) {
            ...
            AlpineComponentLoader._observability.emit(eventName, eventDetail, {
                tagName
            });
            this.dispatchEvent(new CustomEvent(`acl:${eventName}`, {
                bubbles: true,
                composed: true,
                detail: eventDetail
            }));
        ...
        async _resolveContent(source) {
            const content = await AlpineComponentLoader.loadTemplate(source, settings), info = AlpineComponentLoader.getTemplateLoadInfo(source, settings);
