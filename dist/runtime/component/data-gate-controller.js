...
}, importLocalModule = (specifier)=>import(/* @vite-ignore */ resolveLocalModule(specifier)), importDeferredLocalModule = (specifier)=>import(/* @vite-ignore */ resolveLocalModule(specifier)), [{ ACLLoadError, toACLLoadError }, { readBooleanAttribute, readNumberAttribute }] = await Promise.all([
    ...
    return class extends Base {
        async _ensureDataRuntime() {
            if (!this._dataRuntimePromise) {
                const loading = loadDataRuntime().then(// Install the concrete data controller after its modules arrive
                ({ controller, cache })=>{
                    AlpineComponentLoader._dataRuntime = cache;
                    const DataController = controller.withComponentData(class {
                    }, context);
                    ...
                    for (const name of Reflect.ownKeys(DataController.prototype)){
                        ...
                        Object.defineProperty(this, name, Object.getOwnPropertyDescriptor(DataController.prototype, name));
                    }
                    return this;
                }).catch((error)=>{
