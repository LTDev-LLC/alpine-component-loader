...
export const redactRuntimeDetail = (value, key = '', seen = new WeakSet())=>{
    ...
    seen.add(value);
    if (Array.isArray(value)) return value.slice(0, 50).map(// Transform the current item
    (item)=>redactRuntimeDetail(item, key, seen));
    return Object.fromEntries(Object.entries(value).map(// Transform the current item
    ([childKey, child])=>[
            childKey,
            redactRuntimeDetail(child, childKey, seen)
        ]));
};
const emptyMetrics = ()=>{
...
export const createObservability = ()=>{
    ...
    let options = false, sequence = 0, metrics = emptyMetrics();
    ...
    const emit = (type, detail = {}, defaults = {})=>{
        ...
        const safeDetail = redactRuntimeDetail(detail), record = {
            sequence: ++sequence,
            timestamp: Date.now(),
            type,
    ...
    const report = (level, message, error = null, context = {})=>{
        ...
        try {
            ...
            else console?.[level]?.(message, ...error == null ? [] : [
                error
            ]);
        } catch  {
