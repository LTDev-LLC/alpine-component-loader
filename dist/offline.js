...
const dispatch = (name, detail)=>{
    // Dispatch
    if (typeof window !== 'undefined' && typeof CustomEvent !== 'undefined') window.dispatchEvent(new CustomEvent(`acl:offline-${name}`, {
        detail
    }));
};
...
export const registerOfflineWorker = async (url = '/acl-sw.js', options = {})=>{
    if (typeof navigator === 'undefined' || !navigator.serviceWorker) throw new TypeError('[ACL Offline] Service workers are not available in this environment.');
...
export const getOfflineStatus = async ()=>{
    ...
    const registrations = await navigator.serviceWorker.getRegistrations(), estimate = await navigator.storage?.estimate?.();
    return {
        supported: true,
        ...
        storage: estimate ? {
            ...
            remaining: Number.isFinite(estimate.quota) && Number.isFinite(estimate.usage) ? Math.max(0, estimate.quota - estimate.usage) : null
        } : null,
        registrations: registrations.map(// Transform the current item
        (registration)=>({
                scope: registration.scope,
